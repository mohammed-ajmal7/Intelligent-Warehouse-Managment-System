...
class IMS:
    def __init__(self, root):
        ...
        # Clock Frame
        self.lbl_clock = Frame(self.root, bg="grey")  # Use a Frame instead of Label for clock and message icon
        self.lbl_clock.place(x=0, y=70, relwidth=1, height=30)

        # Clock text inside the frame
        self.clock_text = Label(self.lbl_clock, text="Welcome to SMART WAREHOUSE MANAGEMENT\t\t Date:DD-MM-YYYY\t\t Time:HH:MM:SS", font=("times new roman", 15), bg="grey", fg="white")
        self.clock_text.pack(side=LEFT, padx=10)

        # Load the message icon image
        self.message_icon = Image.open("images/message.png")  # Replace with your image path
        self.message_icon = self.message_icon.resize((20, 20), Image.Resampling.LANCZOS)  # Resize the icon
        self.message_photo = ImageTk.PhotoImage(self.message_icon)

        # Add the message icon to the clock frame
        btn_message = Button(self.lbl_clock, image=self.message_photo, command=self.messenger, bg="white", cursor="hand2", bd=0)  # Remove border
        btn_message.pack(side=RIGHT, padx=10)  # Place the icon on the right side of the frame
    ...
    def messenger(self):
        # Logic for the message box or message feature
        messagebox.showinfo("Messenger", "This is where the messenger feature would work!")
